...
#
# 2017-6-13
# stockdemo_x_v3_CNN.py
# Add CNN in Sequential model
...
#model.add(Conv1D(input_shape=(seq_steps, x_train.shape[-1]), filters=32, kernel_size=3, padding='same', activation='relu'))
...
for i in range(len(x_test)):
    pred_test_close[seq_steps+i] = pred_test_close[seq_steps+i-1] * (1 + p[i] / 100 * 0.5)
...
plt.figure(figsize=(16,5))
plt.plot(pred_close_log,color='red', label='prediction')
plt.plot(data_close_log,color='blue', label='y_train')
plt.title('price(log) prediction')
plt.legend(loc='upper left')
plt.show()
